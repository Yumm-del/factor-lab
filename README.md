...
**让 AI 帮你挖 A 股因子：一句话描述想法 → 受限因子表达式 → 机构级体检 → AI 解读 → 一键构建策略**
...
```
自然语言想法              受限 DSL 表达式               机构级体检                AI 解读              策略
"放量后延续上涨" ──LLM──▶ rank(量比5/20 × 动量20) ──自动──▶ IC / RankIC  ──LLM──▶ 大白话报告  ──▶ Top-30组合
                           ↓ 白名单算子 + 深度限制             分层回测        + 改进建议       扣成本、对比
                           （不能执行任意代码）                 换手率                             沪深300指数
                                                                IC衰减曲线
                                                                综合评分
```
...
- **数据**：baostock 沪深300 成分股日线（前复权，3 年约 750 个交易日），含 close/high/low/volume/amount/turn/pe/pb，另含沪深300指数（策略基准），全部真实数据
...
- **综合评分**：IC 强度（30 分）＋ 信息比率（25 分）＋ 多空年化（30 分）＋ 层间单调性（15 分），用 tanh 压缩防止过拟合因子刷分；判定标注因子方向（正/反）
- **策略构建**：Top-N 等权组合、周频调仓、双边成本扣减、与真实沪深300指数对比（年化/Sharpe/回撤/超额/信息比率/胜率）
- **LLM**：DeepSeek API（`deepseek-v4-pro`），输出受 JSON Schema 校验 + 深度/节点数限制，非法输出自动重试；预置示例支持离线演示

## 演示剧本

答辩 5 分钟讲法见 [docs/demo_script.md](docs/demo_script.md)——包含每 30 秒的台词、动作和评委追问应答表。
