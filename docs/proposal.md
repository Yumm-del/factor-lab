...
**结论——市场空白确认**：以「结构性安全 DSL（对比约定式安全）+ 全流程可解释（对比黑盒）+ 全 A 真实数据双池对照（对比美股/小样本）+ 开源可复现（对比闭源订阅）」为组合的工作台，在 A 股个人研究者的定位上仍缺位；表中 8 个维度里，**没有第二个项目同时具备「结构式安全 + 双池对照 + 反思迭代」三项**。本项目以 18 个白名单算子组成的受限表达式树为安全地基，补上这一环。
...
<div class="callout info"><h4>个人使用场景实录（2026-08 第一人称实测）</h4>
作为量化初学者，我第一次完整体验这个闭环只用了约 30 分钟：打开工作台 → 输入一句话想法「连续放量之后，股价短期还能延续上涨吗？」→ 选择沪深300 池 → AI 生成受限表达式（约 40 秒）→ 体检报告显示 IC 显著但换手率偏高、评分进入达标档 → 一键进入策略构建 → 生成 Top-30 周频组合净值曲线，与沪深300 指数同屏对比。全程没有写一行代码，也没有配置任何 API；每一步的结果（表达式、八项指标、迭代轨迹）都能点开查看，看不懂的地方 AI 给出中文解读——「想法 → 可验证结论」的直达体验，正是两道门槛（生成门槛 + 验证门槛）被拆掉之后的样子。</div>
...
<div class="timeline">
  ...
  <div class="tl-item"><b>阶段二 · 2026-10~11</b><span>v0.9 已于 2026-08 开源（GitHub: Yumm-del/factor-lab）；本阶段完成真实用户反馈收敛、全 A 池因子库扩充与 <b>v1.0</b>（2026-10）发布。</span></div>
...
<div class="timeline">
  ...
  <div class="tl-item"><b>2026-10</b><span>v1.0 发布（用户反馈收敛 + 全 A 池因子库扩充）</span></div>
...
**定价锚点**：聚宽 VIP 999 元/年、SVIP 2799 元/年（2025 年调价）；Wind 单终端 39,800 元/年——市场对「研究工具年费」的接受区间已被验证。本项目增值服务定价参考聚宽 VIP 区间：云端全 A 因子库更新订阅 **499 元/年**、批量因子扫描服务 **999 元/年**（拟定），约为聚宽同类能力的 50%–70%——以开源免费版建立用户基数后自然转化，先免费、后付费。

![图 8-2 定价锚点对比](charts/pricing_bar.png)

*图 8-2：研究工具年费锚点对比（2026-08 核实：聚宽 2025 年调价、Wind 官网公开报价；本项目为拟定定价）——本项目两档约为聚宽同类能力的 50%–70%。*
