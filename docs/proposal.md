...
**91 个内置因子（沪深300 全样本）名义显著（|t_HAC|≥2）7 个，BH 校正后 1 个、BY 校正后 1 个**——Alpha#25（WQ101 移植）：IC +0.021、t_HAC=+4.29、q_BH=0.0016、q_BY=0.0081。该因子在早期 IID t 口径下仅 t=3.27、q=0.0975（未过线）——其 IC 序列以负自相关为主，长期方差小于单期方差，普通 t 反而低估显著性；换口径后数字如实变化，这正是统计修正该做的事。**独立复核（2026-09-03）**：该数字已在 BigQuant 独立数据环境复算——动态沪深300 成分（`cn_stock_index_component` 每日成分）、783 个交易日、同一 IC/HAC 口径独立实现（不调用仓库代码）：IC +0.0202、t_HAC=+4.00、p<0.001（本地 +0.021 / +4.29），**符号与量级一致**；微差来自复权口径（BigQuant 行情为「原始价 × adjust_factor」的复权价，复核脚本先还原真实价）与样本截止日（本地至 2026-08-14，BigQuant 至 08-21）。复核脚本 `scripts/bigquant_recheck_alpha25.py`，需在 BigQuant notebook 内运行（数据 API 仅平台可访问），运行记录见 6.2 验证文档第 6 节。
