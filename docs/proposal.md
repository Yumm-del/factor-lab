...
<div class="kpi-grid">
  ...
  <div class="kpi"><div class="kpi-num">+0.0546</div><div class="kpi-label">BigQuant 低波动发现期 5 日 RankIC</div></div>
  <div class="kpi"><div class="kpi-num">5</div><div class="kpi-label">BigQuant 预注册候选，严格留出期筛选</div></div>
...
- **关键实证（2026-08，双环境交叉验证）**：原始 AI「放量延续」在本地样本内策略年化 +14.4%，但在 BigQuant 动态沪深300、周频 Top-30、20bps 成本复核中年化 −2.45%，**未通过接纳门槛并被淘汰**；低波动作为经典对照在 BigQuant 发现期 5 日 RankIC +0.0546、留出期转弱，触发生命周期风险提示（见 6.2）。经典因子库、全 A 双池与口径敏感性细节见第 6 章；
- **差异化（2026-08 全网竞品核实，含 QuantaAlpha 源码逐行核实）**：微软 Qlib/RD-Agent、AlphaEvolve、AlphaGen 等走「LLM 写代码 + 事后防火墙/沙箱」，安全靠**约定**；QuantaAlpha（1.5k stars，2026-01 开源）主路径为受限表达式 DSL + AST 复杂度/一致性正则，但表达式最终仍经 `eval()` 执行、函数白名单靠提示词约定而非解析器校验，且保留无隔离的自由代码副路径（subprocess 直接运行）——安全同为**约定式**而非结构保证；BigQuant 的 AI 选股是黑盒不可审计；国内平台（聚宽/米筐）无 LLM 因子生成；**「解析器级白名单 + 解释器求值（无 eval/exec）的结构安全 DSL + 全流程可解释 + 全 A 真实数据双池对照 + 开源可复现」的组合目前是市场空白**；
...
**结论——市场空白确认**：以「结构性安全 DSL + 全流程可解释 + 全 A 真实数据双池对照 + 开源可复现」为组合的工作台，在 A 股个人研究者定位上仍缺位；表中 8 个维度里，**没有第二个项目同时具备「结构式安全 + 双池对照 + 反思迭代」三项**。本项目以 38 个白名单算子组成的受限表达式树为安全地基，补上这一环。
...
<div class="cards">
  <div class="feature-card"><h4>01 安全受限的因子 DSL</h4><p><b>现状问题</b>：RD-Agent/Qlib 等走「LLM 写 Python 代码 + 事后防火墙」，安全靠<b>约定</b>不靠结构。<br><b>本方案</b>：38 白名单算子 + 表达式树 + 深度/节点上限，<b>解析层拦截非法输入</b>（结构安全，非事后审查）；AI 有创造空间、没有越权空间。<br><b>证据</b>：非法表达式中文报错；LLM 输出校验 + 重试。</p></div>
...
<figure>
<img src="charts/bigquant_lowvol_regime.png" alt="BigQuant低波动月度5日RankIC" style="max-width:165mm">
<figcaption>图 6-2：BigQuant 独立沙箱的低波动因子月度 5 日 RankIC（动态沪深300；橙色虚线为留出验证期起点）。2026 年正负交替，而非单向衰减，说明因子存在明显的市场状态依赖。</figcaption>
</figure>

<figure>
<img src="charts/bigquant_lowvol_regime_raw.jpg" alt="BigQuant 低波动月度诊断原始运行截图" style="max-width:165mm">
<figcaption>图 6-2A：BigQuant 独立沙箱原始运行截图（2026-08-31）。截图上方保留月度统计表，下方为同一段代码输出的月度 5 日 RankIC 图；虚线右侧为预先划定的留出验证期。</figcaption>
</figure>
...
<figcaption>图 6-2B：BigQuant 独立沙箱的预注册候选筛选原始结果（2026-08-31）。低波动仅在发现期通过、留出期未通过；其余候选在发现期或留出期至少一段未达到预设门槛，最终 0/5 通过双阶段筛选。</figcaption>
</figure>
...
因子有效性高度依赖股票池与数据环境——沪深300 上有效的动量因子，在全 A 追高反而被反转；低波动因子也呈现市场状态依赖；量价候选即使本地分层为正，仍须经 BigQuant 等独立环境复核后才可接纳。<b>不能把单一池子的结论直接迁移。</b>同时口径本身也影响结论：同一因子在近 252 交易日窗口与全样本 3 年窗口的评分可以相差 3 倍以上（放量延续全 A：60.1 → 19.6，alpha 集中在 2025-26）；行业/市值中性化同样会改变强弱甚至方向（低波动全 A 中性化后评分 15.6 → 35.2）——这正是系统默认展示「近 252 日窗口 + 全样本滚动 IC 生命周期」双视角的原因：<b>结论必须连同池子、窗口、口径一起解读</b>。</div>
