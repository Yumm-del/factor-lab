...
### 6.5.2 因子池 FDR 多重检验校正（2026-08 新增）

挖 91 个因子只看最好看的，运气也会选出「显著」的——α=0.05 下 91 个纯噪声因子期望产生约 4.5 个假显著。为此系统新增 Benjamini-Hochberg 程序（q 值递推，无外部统计库依赖）：**91 个内置因子（沪深300 全样本）名义显著（|t|≥2）6 个，经 FDR 校正后 0 个**——最佳 Alpha#25：t=+3.27，p=0.0011，q=0.0975，未过 0.05 门槛。

这个「不好看」的数字恰恰是诚实性的证据：**系统不会因为挖得多就宣称有效**。校正后 q 值将作为阶段 2 因子合成层的入池准入门槛（因子越多、门槛越高，这是多重检验校正的意义）。诚实边界：q=0.0975 已接近门槛，0 显著 ≠ 91 个因子全部无效，而是「现有 3 年样本 + 单池检验下，尚不足以把任何一个因子与噪声区分开」——把「运气好」与「真有效」分开需要更多样本（OOS 动态跟踪持续积累中，见 6.6 第 1 条）。

**双池对照（2026-09 全 A 补齐 open 字段后实测）**：同样 91 个因子在全 A 池（5320 只）**名义显著 30 个、经 FDR 校正后 27 个**（t 值 4.7~6.3，q≈0，最高 t 值 Alpha#47：IC −0.026、t=−6.18）——与沪深300 的「6 → 0」不是矛盾，而是**统计功效的体现**：截面越大，因子值与未来收益的截面相关估计越稳；同样的信号在 300 只的机构核心池里被噪声淹没，在 5320 只的全 A 池里浮现出来。这与 6.2 双池对照结论一致——**结论必须连同池子一起解读**。同时诚实标注：27 个通过 FDR 只回答「信号不是运气」，**不回答「可交易」**——可交易性还须过分层单调、换手、成本与样本外门槛（OOS 跟踪持续积累中），FDR 是入池准入门槛而非接纳证据。
