# -*- coding: utf-8 -*-
"""
BigQuant 独立复核：Alpha#25 的 HAC 稳健显著性（对照项目书 6.5.2）
=================================================================
目的：在 BigQuant 独立数据环境复算 Alpha#25 的全样本 IC 显著性，
      检验本地 baostock 数据上得到的 HAC 结论是否换环境仍成立。

口径（与 factor_lab/robust.py 完全一致）：
  - IC：每日截面 Pearson 相关（t 日因子值 vs t+1 日收益），
        当日有效股票 < 10 只记 NaN
  - HAC：Newey-West Bartlett 核，带宽 L = max(1, int(8*(n/100)^(2/9)))
  - p 值：正态近似双尾 erfc(|t|/sqrt(2))；|t_HAC| >= 2 记名义显著

对照目标（本地 baostock 前复权 2023-06-01~2026-08-14，动态沪深300）：
  Alpha#25：IC +0.021、t_HAC +4.29、q_BH 0.0016、q_BY 0.0081
  ——91 因子池名义显著 7 个中，BH/BY 校正后唯一存活者。

已实测基线（BigQuant 数据环境 2026-09-03 跑通）：
  IC +0.0202、t_HAC +4.00、p=0.0001、有效天数 763
  本次重跑应复现同量级数字；若 t_HAC 掉到 2 以下，如实记录并回贴输出。

用法：AIStudio 新建 notebook，把本文件内容整段粘进一个 cell，Shift+Enter 运行。
"""

# ===================== import（全部前置，避免逐 cell 运行缺命名空间） =====================
...
# ===================== 第 1 步：数据探查 =====================
# 确认 cn_stock_bar1d 实际列名（以平台表结构为准）。
# 注意：dai.query 返回 QueryResult，须 .df() 转 pandas；
#       filters 是 dict（date 分区过滤），不带分区会全表扫描报错。
probe = dai.query(
...
# ===================== 第 2 步：取数（动态沪深300 成分日线） =====================
# 沪深300 每日成分来自 cn_stock_index_component（index_code 000300.SH，
# 含每日调样），join 行情表。SQL 与 filters 都限 date 分区，避免全表扫描。
df = dai.query(
    """
...
# ===================== 第 3 步：宽面板 + Alpha#25 因子（独立翻译） =====================
# Alpha#25 DSL: rank( neg(ts_returns(close,1)) * ts_mean(volume,20) * vwap * (high - close) )
#   逐算子对应（每步按股票滚动，最后截面 rank——与 dsdl 求值顺序一致）：
#     ts_returns(close,1) = close/delay(close,1) - 1   （1 日收益）
#     ts_mean(volume,20)  = volume 的 20 日滚动均值
#     rank(·)             = 每日截面升序排名（截面算子）
def to_panel(series_name: str) -> pd.DataFrame:
    """长表 → 宽面板（行=date，列=instrument），缺失留 NaN。"""
...
#   后复权价（放大数百倍，各股量级不同）——直接进截面 rank 会被高 adj 股票
#   主导排序 → 因子失真。还原真实价：P_raw = P_adj / adjust_factor。
#   收益用还原价计算 → 除权日无跳空；volume/amount 是真实值，直接用。
close_raw = close_p / adj_p
high_raw  = high_p / adj_p
# vwap：表无 vwap 列 → amount/volume 近似当日均价（真实价尺度）
...
def hac_tvalue(x: np.ndarray) -> float:
    """Newey-West HAC 稳健 t（Bartlett 核）。样本 <30 返回 NaN。"""
    x = x[~np.isnan(x)]
    n = len(x)
    if n < 30:
        return float("nan")
...
if not math.isnan(t_hac) and abs(t_hac) >= 2:
    ...
    print("  → 独立复核未复现显著！如实记录差异，把本输出贴回给 Claude 排查：")
