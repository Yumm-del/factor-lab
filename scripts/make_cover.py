...
# 变体：python make_cover.py a → 带图框版；python make_cover.py b → 无框版（默认，定稿）
VARIANT = sys.argv[1] if len(sys.argv) > 1 else "b"
OUT = os.path.join(ROOT, "docs", "cover_16x9.png" if VARIANT == "b" else f"cover_{VARIANT}.png")
W, H = 1920, 1080
...
def draw_nav_chart(d: ImageDraw.Draw, nav: pd.Series, bench: pd.Series,
    ...
    # —— 图题（a 变体在框内顶部；b 变体在图上缘）——
    f_cap = font(r"C:\Windows\Fonts\simsun.ttc", 25)
...
def main() -> None:
    ...
    # —— 字体（宋体正文/标题、Times 数字英文）——
    f_title = font(r"C:\Windows\Fonts\simsun.ttc", 122)   # 大标题（宋体，与英文副题拉开层级）
    f_en = font(r"C:\Windows\Fonts\times.ttf", 48)        # 英文副题（衬线）
    f_sub = font(r"C:\Windows\Fonts\simsun.ttc", 34)      # 中文副题
    f_tag = font(r"C:\Windows\Fonts\simhei.ttf", 27)      # 赛事标签（黑体小字）
    f_feat = font(r"C:\Windows\Fonts\simsun.ttc", 27)     # 特性行
    f_foot = font(r"C:\Windows\Fonts\simsun.ttc", 27)     # 底部信息

    # —— 页眉红线（研报页眉惯例，贯穿全宽）——
    d.line([120, 78, 1800, 78], fill=RED, width=3)
    ...
    # —— 左半文字区（居中偏上，留白充足）——
    cx = 480
    d.text((cx, 330), "因子实验室", font=f_title, fill=INK, anchor="mm")
    d.text((cx, 462), "AI Factor Lab", font=f_en, fill=RED, anchor="mm")
    d.text((cx, 560), "AI 驱动的 A 股因子挖掘与分析工作台",
           font=f_sub, fill=INK, anchor="mm")

    # —— 特性行（标题区下方，弱化）——
    d.text((cx, 760), "安全受限表达式  ·  IC/分层/换手/衰减  ·  全 A 双池 5300+",
    ...
    # —— 底部横线 + 署名（研报页脚风）——
    d.line([120, 980, 1800, 980], fill=GRID, width=2)
    d.text((120, 1020), "团队：溯因量化  ·  参赛者：吕滢滢（广东金融学院）  ·  2026 年 9 月",
           font=f_foot, fill=MUTED)

    img.save(OUT, "PNG")
    print(f"已生成: {OUT}（{W}×{H}，16:9）")
