...
"""
"""参考实现兼容层（pandas 2.x 适配版）：移除 sqlalchemy/MySQL 依赖，
    6 处 pd.Panel（pandas 2.x 已移除）改 np.maximum/minimum（语义等价，
    NaN 区域差异由对拍脚本排除）。"""
...
class Alpha101(object):
    def __init__(self, df_data):
        ...
        # 原版 STHSF 复制粘贴 bug：alpha002_2 误绑定为 alpha002_1（合成层
        # 退化为 corr(x,x)=-1 常数）→ 适配层修正，对拍恢复真实 #2 结构
        self.alpha002_2 = df_data['alpha002_2']
    ...
    def alpha071(self):
        ...
        # 原代码用 pd.Panel 逐行取 max（Panel 已在 pandas 2.x 移除）→ np.maximum 等价
        return pd.DataFrame(np.maximum(self.alpha071_1, p2),
                            index=self.alpha071_1.index, columns=self.alpha071_1.columns)
    ...
    def alpha073(self):
        ...
        return -1 * pd.DataFrame(np.maximum(p1, self.alpha073_2),
    ...
    def alpha077(self):
        ...
        return pd.DataFrame(np.minimum(p1, p2),
                            index=p1.index, columns=p1.columns)
    ...
    def alpha088(self):
        ...
        return pd.DataFrame(np.minimum(p1, self.alpha088_1),
                            index=p1.index, columns=p1.columns)

    def alpha092(self):
        alpha092_2 = ts_rank(decay_linear_pn(correlation(rank(self.low), rank(self.adv30), 8), 7), 7)
        return pd.DataFrame(np.minimum(self.alpha092_1, alpha092_2),
                            index=self.alpha092_1.index, columns=self.alpha092_1.columns)
    ...
    def alpha096(self):
        ...
        return -1 * pd.DataFrame(np.maximum(p1, self.alpha096_1),
                                 index=p1.index, columns=p1.columns)
