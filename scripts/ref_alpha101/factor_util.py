...
def sum(df, window=10):
    """官方代码多处直接调用 sum()，原版 factor_util 将其定义为 ts_sum 别名。"""
    return ts_sum(df, window)
...
def rolling_rank(na):
    ...
    """
    # 平均秩（与 scipy rankdata method='average' 等价），避免 scipy 依赖
    return pd.Series(na).rank(method="average").iloc[-1]
...
def decay_linear(df, period=10):
    ...
    return pd.DataFrame(na_lwma, index=df.index, columns=df.keys())  # 原版为 ['CLOSE']（Series 输入），面板化后保留真实列名
