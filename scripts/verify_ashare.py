...
print(f"[{time.time()-t0:.0f}s] 全A池 FDR：{res['n_tested']} 因子 → "
      f"名义 |t|≥2 若干 → 校正后显著 {res['n_significant']} 个（α=0.05）")
...
for r in rows[:8]:
    print(f"  {r['name']:<16} IC {r['ic_mean']:+.4f}  t {r['t']:+6.2f}  "
          f"p {r['p']:.4f}  q {r['q']:.4f}  {'★' if r['significant'] else ''}")
print(f"  → 名义显著 {len(rows)} 个；校正后 {res['n_significant']} 个")
